...
class FC_ScannerView: UIView {
    ...
    init(frame: CGRect, config: QRCodeCompat) {
    ...
    @objc private func flashlightClicked(button: UIButton) {
        ...
        fc_setFlashlight(on: button.isSelected)
    }
...
// MARK: - 扫描线条动画
extension FC_ScannerView {
    
    /** 添加扫描线条动画 */
    func fc_addScannerLineAnimation() {
        // 若已添加动画，则先移除动画再添加
    ...
    /** 暂停扫描器动画 */
    func fc_pauseScannerLineAnimation() {
        // 取出当前时间，转成动画暂停的时间
...
extension FC_ScannerView {
    ...
    /** 显示手电筒 */
    func fc_showFlashlight(animated: Bool) {
        if animated {
    ...
    /** 隐藏手电筒 */
    func fc_hideFlashlight(animated: Bool) {
...
extension FC_ScannerView {
    ...
    /** 移除指示器 */
    func fc_removeActivityIndicator() {
        if self.activityIndicator != nil {
...
extension FC_ScannerView {
    ...
    /** 设置手电筒开关 */
    func fc_setFlashlight(on: Bool) {
        QRCodeHelper.fc_flashlight(on: on)
