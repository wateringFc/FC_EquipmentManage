...
class FC_DownMenu: UIView {
    ...
    var isHiddenLine: Bool = false {
        didSet{
            topLine.isHidden = isHiddenLine
            bottomLine.isHidden = isHiddenLine
        }
    }
    /// 按钮数组
    ...
    fileprivate lazy var topLine: UIView = {
        let topLine = UIView(frame: CGRect(x: 0, y: 0, width: self.width, height: 1))
        topLine.backgroundColor = Line_e0e0e0_Gray
        return topLine
    }()
    
    fileprivate lazy var bottomLine: UIView = {
        let bottomLine = UIView(frame: CGRect(x: 0, y: self.height - 1, width: self.width, height: 1))
        bottomLine.backgroundColor = Line_e0e0e0_Gray
        return bottomLine
    }()
...
extension FC_DownMenu {
    ...
    fileprivate func setupUI() {
        ...
        for i in 0..<titleArray.count {
            ...
            selectBut.setTitleColor(Main_222222_Color, for: .normal)
...
class DownMenuCell: UITableViewCell {
    ...
    lazy var contentLb: UILabel = {
        ...
        contentLb.textColor = Main_222222_Color
        return contentLb
    }()
    ...
    var isSelecteds: Bool = false {
        didSet{
            if isSelecteds {
                contentLb.textColor = Theme_4B77F6_Blue
