...
extension String {
    ...
    /// 正则匹配用户密码8-32位（数字+字母+字符 or 数字+字母 or 数字+字符 or 字母+字符）
    var is8_32Paswd: Bool {
        return isMatch("^((?!\\d+$)(?![a-zA-Z]+$)[a-zA-Z\\d@#$%^&_+].{7,32})+$")
    }
...
extension UIBarButtonItem {
    ...
    ///   - target: 持有者
    ///   - action: 事件
    func initWithImage(image: String, highImage: String, target:AnyObject, action:Selector) -> UIBarButtonItem {
        let but = UIButton(type: .custom)
        but.setBackgroundImage(imageNamed(image), for: .normal)
        but.setBackgroundImage(imageNamed(highImage), for: .highlighted)
    ...
    ///   - target: 持有者
    ///   - action: 事件
    func initItemWithTitle(title: String, color: UIColor, font: UIFont, target:AnyObject, action:Selector) -> UIBarButtonItem {
        let but = UIButton(type: .custom)
        but.setTitle(title, for: .normal)
        but.setTitleColor(color, for: .normal)
