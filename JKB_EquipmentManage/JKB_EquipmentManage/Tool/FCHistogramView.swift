...
class FCHistogramView: UIView {
    ...
    /// x周文字是否旋转， 默认不旋转
    var isXAxisLabsRotating: Bool = false
    /// 如果旋转，需要设置该值, 默认旋转90度
    var rotatingAngle:CGFloat = CGFloat.pi/2
    /// 柱状图显示的总宽度
    ...
    override func layoutSubviews() {
        ...
        // 单位标签
        unitLab.frame = CGRect(x: 5, y: 0, width: 60, height: 20)
        // 总宽度
        _totalWidth = gapWidth + (barWidth + gapWidth) * CGFloat(xValueArr!.count)
        // 总高度
        _totalHeight = scrollView.height - 20 - scrollView.height/2
        // 设置滑动范围
        scrollView.contentSize = CGSize(width: 30 + _totalWidth!, height: 0)
        // 设置内容视图
        contentView.frame = CGRect(x: 30, y: 30, width: _totalWidth!, height: _totalHeight!)
        // 绘制柱状图
        drawHistogramView()
    }
}
...
extension FCHistogramView {
    ...
    fileprivate func addXAxisLabs() {
        for i in 0..<xValueArr!.count {
            ...
            label.textColor = Sub_666666_Color
            if isXAxisLabsRotating {
                label.textAlignment = .left
                label.bounds =  CGRect(x: 0, y: 0, width: 200, height: 20)
                label.center = CGPoint(x: point!.x, y: point!.y + label.height + 85)
                label.transform = CGAffineTransform(rotationAngle: rotatingAngle)
            }else {
